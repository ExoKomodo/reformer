        ...
        (cond
         ((bytevector? val)
          (bind-blob p idx (bytevector->pointer val) (bytevector-length val)
                     sqlite-transient))
         ;; BEGIN: Added by James Orson on August 18, 2024
         ((symbol? val)
          (sqlite-bind stmt key (symbol->string val)))
         ;; END
         ((string? val)
...
;; NOTE: From here onwards, everything in this file is added by developers of reformer.fyi

;; Source: https://notabug.org/guile-sqlite3/guile-sqlite3/src/5fa25ef90859a9c6c3a53ccbfd2a4b62b9ac538a/tests/basic.scm#L27

(define (sqlite-map-with-index proc stmt)
  (let ((index 1))
    (map-in-order (lambda (row)
                    (proc row index)
                    (set! index (+ index 1)))
                  (reverse! (sqlite-fold cons '() stmt)))))

(define (-row-print-indexed row index)
  (format #t "Row ~3,'0d: ~a~%" index row))

(define (-row-print row)
  (format #t "~a~%" row))

(define* (sqlite-exec* db sql #:key (parameters '()) (row-handler #f) (indexed #f))
